...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    let params: any[] = [cleanIdentifier];
...
router.get('/me', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [users]: any = await pool.query(
      'SELECT id, name, email, role, phone, ci, address, client_type, photo_url, permissions, business_id, created_at FROM users WHERE id = ?',
      [req.user.id]
    );
    if (users.length === 0) {
      return res.status(404).json({ message: 'Usuario no encontrado' });
...
router.put('/profile', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (email && email.trim().toLowerCase() !== currentUser.email.toLowerCase()) {
      const [emailExists]: any = await pool.query('SELECT id FROM users WHERE email = ? AND id != ?', [email.trim().toLowerCase(), req.user.id]);
    ...
    await pool.query(
      `UPDATE users 
       ...
       WHERE id = ?`,
      [name.trim(), newEmail, newPhone, newCi, newAddress, newPhoto, updatedPasswordHash, req.user.id]
    );
    ...
    const token = jwt.sign(
      { id: updatedUser.id, email: updatedUser.email, role: updatedUser.role, name: updatedUser.name, business_id: updatedUser.business_id || 1 },
      JWT_SECRET,
      { expiresIn: '8h' }
    );
...
router.post('/profile/photo', authenticate, uploadAvatar.single('photo'), async (req: AuthRequest, res: Response) => {
  try {
    if (!req.file) {
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    let emailSent = false;
    try {
      emailPreviewUrl = await sendPasswordResetEmail(user.email, user.name, code);
      emailSent = true;
    } catch (mailError) {
