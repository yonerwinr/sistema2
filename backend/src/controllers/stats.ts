...
router.get('/reports', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    `;
    const salesParams: any[] = [startDateStr, endDateStr];
    ...
    if (seller_id && seller_id !== '') {
      if (seller_id === 'online') {
        ...
        salesParams.push(parseInt(seller_id as string));
      }
    }
    salesQuery += " ORDER BY s.created_at DESC";

    const [sales]: any = await pool.query(salesQuery, salesParams);
    ...
    let paymentQuery = `
      ...
      WHERE s.status = 'completed' AND s.is_quotation = 0 AND s.created_at >= ? AND s.created_at <= ?
    `;
    const paymentParams: any[] = [startDateStr, endDateStr];
    if (seller_id && seller_id !== '') {
      if (seller_id === 'online') {
        paymentQuery += " AND s.type = 'online'";
      } else {
        paymentQuery += " AND s.seller_id = ? AND s.type = 'pos'";
        paymentParams.push(parseInt(seller_id as string));
      }
    }
