...
async function getExchangeRates(): Promise<ExchangeRates> {
  const [rows]: any = await pool.query(
    ...
    ['usd_to_ves_rate', 'eur_to_ves_rate']
  );

  const values: ExchangeRates = { usdToVes: 40, eurToVes: 43.5 };
...
function calculateAmountVes(amount: number, currency: string, rates: ExchangeRates): number {
  const normalizedCurrency = (currency || 'USD').toUpperCase();
  if (normalizedCurrency === 'USD') return Number((amount * rates.usdToVes).toFixed(2));
  if (normalizedCurrency === 'EUR') return Number((amount * rates.eurToVes).toFixed(2));
...
router.get('/', authenticate, isAdmin, async (_req: AuthRequest, res: Response) => {
  try {
    const [expenses]: any = await pool.query(
      'SELECT * FROM expenses ORDER BY is_active DESC, next_due_date IS NULL, next_due_date ASC, created_at DESC'
    );

    res.json(expenses.map((expense: any) => ({
      ...expense,
...
router.post('/', authenticate, isAdmin, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const [result]: any = await pool.query(
      ...
      [name, description || null, amountValue, amountVes, normalizedCurrency, normalizedType, normalizedActive, effectiveStartDate, effectiveNextDueDate]
    );
...
router.put('/:id', authenticate, isAdmin, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const [existingRows]: any = await pool.query('SELECT * FROM expenses WHERE id = ?', [id]);
    ...
    await pool.query(
      ...
      [name || current.name, description !== undefined ? description : current.description, amountValue, amountVes, normalizedCurrency, normalizedType, normalizedActive, effectiveStartDate, effectiveNextDueDate, id]
    );
