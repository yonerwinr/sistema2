...
router.post('/checkout', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (couponCode) {
      ...
      const [coupons]: any = await conn.query('SELECT * FROM coupons WHERE code = ? AND active = 1 FOR UPDATE', [cleanCoupon]);
      if (coupons.length === 0) {
...
router.post('/coupon/validate', async (req, res) => {
  ...
  try {
    const [coupons]: any = await pool.query('SELECT * FROM coupons WHERE code = ? AND active = 1', [code.toUpperCase().trim()]);
    if (coupons.length === 0) {
    ...
    if (coupon.user_id !== null) {
      ...
      if (userId) {
        const [alreadyUsed]: any = await pool.query(
...
router.post('/coupons', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    await pool.query('INSERT INTO coupons (code, discount_percent, user_id) VALUES (?, ?, ?)', [
...
router.put('/coupons/:id', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const [coupons]: any = await pool.query('SELECT * FROM coupons WHERE id = ?', [id]);
    if (coupons.length === 0) {
    ...
    await pool.query(
      'UPDATE coupons SET code = ?, discount_percent = ?, active = ?, user_id = ? WHERE id = ?',
      [newCode, newDiscount, newActive, newUserId, id]
    );
...
router.delete('/coupons/:id', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const [result]: any = await pool.query('DELETE FROM coupons WHERE id = ?', [id]);
...
router.get('/settings/rates', async (_req, res) => {
  try {
    ...
    const binanceRate = rows.find((r: any) => r.settings_key === 'binance_usd_to_ves_rate')?.settings_value || '44.50';
...
router.put('/settings/rates', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (binanceUsdToVes !== undefined) {
      await pool.query(
        'INSERT INTO settings (settings_key, settings_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE settings_value = ?',
        ['binance_usd_to_ves_rate', binanceUsdToVes.toString(), binanceUsdToVes.toString()]
      );
    }
...
router.get('/coupons/all', authenticate, async (_req: AuthRequest, res: Response) => {
  try {
...
router.post('/test-smtp', authenticate, async (req: AuthRequest, res: Response) => {
  if (req.user?.role !== 'admin') {
