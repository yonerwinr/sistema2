 ...
 */
export async function runMultiTenantMigration(conn: PoolConnection): Promise<void> {
  ...
  await conn.query(`
    CREATE TABLE IF NOT EXISTS businesses (
      ...
      license_plan VARCHAR(50) DEFAULT 'pro',
  ...
  try {
    const [existingSuper]: any = await conn.query("SELECT id FROM users WHERE email = 'superadmin@facilito.com' LIMIT 1");
