...
exports.provisionBusinessSheet = provisionBusinessSheet;
exports.appendSaleToBusinessSheet = appendSaleToBusinessSheet;
 ...
 */
async function provisionBusinessSheet(businessName, ownerEmail) {
 ...
 */
async function appendSaleToBusinessSheet(businessId, sale, items) {
    try {
        const [rows] = await db_1.default.query('SELECT google_sheet_id, google_sheets_webhook_url, name FROM businesses WHERE id = ? LIMIT 1', [businessId]);
        if (rows.length === 0)
            return;
