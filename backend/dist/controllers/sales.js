...
router.post('/checkout', async (req, res) => {
    ...
    try {
        ...
        // Registrar la venta
        const [saleResult] = await conn.query('INSERT INTO sales (user_id, customer_name, customer_email, customer_phone, total, payment_method, type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
            userId || null,
...
router.get('/history', auth_1.authenticate, async (req, res) => {
    ...
    try {
        const [sales] = await db_1.default.query('SELECT * FROM sales WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
        res.json(sales);
    }
    catch (error) {
