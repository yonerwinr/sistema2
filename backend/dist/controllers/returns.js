...
const canManageReturns = (req, res, next) => {
    if (!req.user) {
...
router.get('/stats', auth_1.authenticate, canManageReturns, async (_req, res) => {
    try {
...
router.get('/', auth_1.authenticate, canManageReturns, async (req, res) => {
...
router.get('/:id', auth_1.authenticate, canManageReturns, async (req, res) => {
    const { id } = req.params;
    try {
...
router.post('/', auth_1.authenticate, canManageReturns, async (req, res) => {
    const { type = 'warranty', sale_id, product_id, product_name, product_sku, customer_name, customer_phone, customer_ci, customer_email, reason, issue_description, status = 'pending', is_repaired = false, repair_cost = 0, repair_notes, technician_name, resolution } = req.body;
    ...
    try {
        ...
        const [lastRows] = await conn.query('SELECT id FROM returns_claims ORDER BY id DESC LIMIT 1');
        ...
        const [result] = await conn.query(`INSERT INTO returns_claims (
...
router.put('/:id', auth_1.authenticate, canManageReturns, async (req, res) => {
    const { id } = req.params;
    const { type, product_name, product_sku, customer_name, customer_phone, customer_ci, customer_email, reason, issue_description, status, is_repaired, repair_cost, repair_notes, technician_name, resolution } = req.body;
    try {
        ...
        await db_1.default.query(`UPDATE returns_claims SET 
...
router.delete('/:id', auth_1.authenticate, auth_1.isAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const [existing] = await db_1.default.query('SELECT code FROM returns_claims WHERE id = ?', [id]);
        if (existing.length === 0) {
            return res.status(404).json({ message: 'Reclamo o devolución no encontrado' });
        }
        const code = existing[0].code;
        await db_1.default.query('DELETE FROM returns_claims WHERE id = ?', [id]);
        (0, audit_1.logAuditEvent)({
            userId: req.user?.id,
            userName: req.user?.name,
            userRole: req.user?.role,
